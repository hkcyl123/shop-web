    ...
    methods: {
      ...
      async deleteOrder(order_id, order_status, index) {
        ...
        if (order_status !== 4 && order_status !== 5) {
          ...
          let result = await this.$axios.post('/order/deleteOrder', {
            order_id
          })
      ...
      async payOrder() {
        ...
        let result = await this.$axios.post('/order/payOrder', {
      ...
      async cancelOrder() {
        ...
        let result = await this.$axios.post('/order/cancelOrder', {
          order_id: this.order_id,
      ...
      async confirmOrder() {
        ...
        let result = await this.$axios.post('/order/confirmOrder', {
          order_id: this.order_id,
          user_id: sessionStorage.getItem('user_id')
        })
        if (result.data.code === 200) {
