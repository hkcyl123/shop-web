    ...
    methods: {
      ...
      async login() {
        ...
        if (!username) {
          ...
          let result = await this.$axios.post('/users/login', {
            username,
            password
          })
          //返回状态码0表示登录成功
          if (result.data.code === 200) {
            ...
            localStorage.setItem('Token', 'Bearer ' + result.data.data)
            // 把用户名保存到store仓库
            let userInf = await this.$axios.get('/users/getUserInfo')
            sessionStorage.setItem("user_id", userInf.data.data.user_id + '')
