  ...
  <div class="goods">
    ...
    <div class="body">
      <div class="category" v-if="categoryObj.category_img">
        <img style="width: 200px; height: 200px" :src="$baseURL + categoryObj.category_img" />
      </div>
      <!-- 列表区 -->
      <div class="list">
        <ul>
...
<script>
  ...
  export default {
    ...
    methods: {
      //获取首页商品列表信息
      async getGoodsList() {
        let { category_id } = this.categoryObj
        let result = await this.$axios.post('/goods/goodsList', {
          category_id
        })
