  ...
  export default {
    ...
    data() {
      return {
        ...
        goodsBanners: '', //商品轮播图
        goodsObj: '', //商品信息
        user_id: sessionStorage.getItem('user_id'), //商品信息
        goodsId: '' //商品id
      }
    },
    activated() {
    ...
    methods: {
      ...
      async getbanner(value) {
        let result = await this.$axios.post('/goods/banners', {
          goodsId: Number(value)
        })
        this.goodsBanners = result.data.data
      },
      //获取商品信息
      async getdetails(value) {
        let result = await this.$axios.post('/goods/details', {
      ...
      async addShoppingCar() {
        this.islogin()
        let { goodsId } = this
        let result = await this.$axios.post('/shoppingCar/add', {
      ...
      async addCollect() {
        this.islogin()
        let { goodsId } = this
        let result = await this.$axios.post('/collection/add', {
